                            ...
                            if mess[0].lower()=='+д':
                                ...
                                try:
                                    ...
                                    if len(mess)==2 and mess[1]=='all':
                                        ...
                                        tokens[a]=[i for i in tokens[a] if i.find('4='+str(int(mess[1])+2000000000))==-1]
                                        text[a]=':'.join(tokens[a])+'\n'
                                    with open("settings", "w+") as file1:
                                            file1.write(''.join(text))
                                except BaseException as e:
                                ...
                                if len(mess)>1:
                                    ...
                                    if 'response' in requs:
                                        requs={"peer_id":upd[3],
                                               ...
                                               "random_id":0,
                                               "access_token":tokens[a][0],
                                               "message":'id этого чата: '+str(int(upd[3])-2000000000)+',название чата: '+requs["response"][0]["title"]}
                                        requests.post("https://api.vk.com/method/messages.send",paramd=requs)
                                    else:
                                        requests.get(f'https://api.vk.com/method/messages.send?peer_id={str(upd[3])}&message=Ошибка!&random_id=0&access_token={tokens[a][0]}&v=5.131')
                                    ...
                                    requs=json.loads(requests.get(f'https://api.vk.com/method/groups.getById?group_ids={str(upd[3])}&access_token={tokens[a][0]}&v=5.131').text)
                                    if 'response' in requs:
                                        requs={"peer_id":upd[3],
                                               "random_id":0,
                                               "v":"5.131",
