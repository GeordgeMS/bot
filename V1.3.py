...
def chktokens():
    global tokens,prcess
    while True:
        with open("settings") as file:
          file=[re.sub("^\s+|\n|\r|\s+$", '', i).split(':') for i in file.readlines()]
        if [i[0] for i in tokens]!=[i[0] for i in file]:
            a=0
            print("Не равны данные в программе и в настройках")
            while a<len(tokens) or a<len(file):
                if len(tokens)>a and ''.join(file).find(tokens[a][0])==-1:
                    tkns1=tokens
                    del tkns1[a]
                    tokens=tkns1
                    tkns1=prcess
                    ...
                    print("Удален токен")
                    prcess=tkns1
                if len(file)>a and ''.join(tokens).find(file[a][0])==-1:
                    tokens.extend([file[a]])
                    Process(target=strt,args=(a,file[a][0],)).start()
                    prcess.append(0)
                    print ("Добавлен токен")
                a+=1
        time.sleep(5)
def strt(a,b):
        global tokens,prcess
        ...
        for jj in range(len(tokens[a][1:])):
            ...
            if jj1[0]=='2':
                ...
                qq+=1
        while len(tokens)>a and tokens[a][0]==b:
            try:
                        ...
                        if len(upd)>6 and upd[0]==4 and (('from' in upd[6] and int(upd[6]['from'])==myid) or ('from' not in upd[6] and upd[2]>1)):
                          ...
                          if 1<len(upd[5])<=100:
                            ...
                            if mess[0].lower()=='+д':
                                ...
                                requests.get(f'https://api.vk.com/method/messages.delete?peer_id={str(upd[3])}&message_ids={upd[1]}&delete_for_all=1&access_token={tokens[a][0]}&v=5.131')
                                if len(mess)==1:
                                    requests.post('https://api.vk.com/method/messages.send',params={"peer_id":str(upd[3]),"message":'Используйте +с [скорость(макс 4)] (id|домен|реплай/либо 0) (иная цель, id чата, либо 0 для выбора текущего чата) (1-вкл тайпинг,0-выкл(по умолчанию)) (шапка, можно не указывать), если следующие части команды не нужны, можно не указывать их как 0!',"random_id":0,"access_token":tokens[a][0],"v":"5.131"})
                                else:
                                    with open("settings") as text:
                                        text=text.readlines() 
                                    bb=1
                                    ...
                                    if len(mess)>3 and mess[3]!='0':
                                        try:
                                          if len(re.findall('c|с',mess[3]))>0:
                                            perid=str(int(mess[3][1:])+2000000000)
                                          else:
                                            perid=mess[3]
                                        except:
                                            pass
                                ...
                                if len(mess)>2 and mess[2]!='0':
                                    try:
                                      if len(re.findall('c|с',mess[2]))>0:
                                ...
                                if len(upd)>=8 and 'reply' in upd[7]:
                                    ...
                                    if len(mess)>1 and mess[1]!='0':
                                            try:
                                                ...
                                                up=0
                                if len(mess)==1:
                                    requests.post("https://api.vk.com/method/messages.send",params={"peer_id":str(upd[3]),"message":'Используйте +а [id|домен|реплай/либо 0 для выбора всех id] (задержка(с)) (id чата, либо 0 для выбора всех чатов) (1-вкл тайпинг,0-выкл(по умолчанию)), если следующие части команды не нужны, можно не указывать их как 0!',"random_id":0,"access_token":tokens[a][0],"v":"5.131"})
                                else:
                                    zd=0
                                    if len(mess)>2:
                                        zd=int(mess[2])
                                    ...
                                    if len(mess)>3 and mess[3]!='0':
                                        try:
                                          if len(re.findall('c|с',mess[3]))>0:
                                            perid=str(int(mess[3])+2000000000)
                                          else:
                                            perid=mess[3]
                                        except:
                                            pass
                                ...
                                requests.get(f'https://api.vk.com/method/messages.delete?peer_id={str(upd[3])}&message_ids={upd[1]}&delete_for_all=1&access_token={tokens[a][0]}&v=5.131')
                                if len(mess)==1:
                                    requests.post("https://api.vk.com/method/messages.send",params={"peer_id":str(upd[3]),"message":'Используйте +т [время(сек)] (id чата/чатов,через запятую,0-выбрать текущий чат) (тайпинг 0-выкл,1-вкл) (шапка), если следующие части команды не нужны, можно не указывать их как 0!',"random_id":0,"access_token":tokens[a][0],"v":"5.131"})
                                else:
                                    with open("settings") as text:
                                        text=text.readlines() 
                                    shp=''
                                    ...
                                    if len(mess)>2 and mess[2]!='0':
                                        try:
                                            perid=[str(int(i[1:])+2000000000) if len(re.findall('c|с',i))>0 else i for i in mess[2].split(',')]
                                        except:
                                            pass
                                    typg='0'
                                    if len(mess)>3 and mess[3]!='0':
                                ...
                                try:
                                    ...
                                    if len(mess)==2 and mess[1]=='all':
                                        ...
                                        perid=[str(int(i[1:])+2000000000) if len(re.findall('c|с',i))>0 else i for i in mess[1].split(',')]
                                        for x in perid:
